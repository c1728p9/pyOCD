...
class PyOCDTool(object):
    ...
    def handle_status(self, args):
        ...
        if isinstance(self.target, pyOCD.target.target_kinetis.Kinetis):
            print "MDM-AP Control: 0x%08x" % self.link.readAP(target_kinetis.MDM_CTRL)
            print "MDM-AP Status:  0x%08x" % self.link.readAP(target_kinetis.MDM_STATUS)
    ...
    def handle_clock(self, args):
        ...
        try:
            ...
            return 1
        self.link.setClock(freq_Hz)

        if self.link.mode == pyOCD.transport.daplink.DAP_MODE_SWD:
            swd_jtag = 'SWD'
        else:
            swd_jtag = 'JTAG'
