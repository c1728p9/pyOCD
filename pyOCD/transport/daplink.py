...
class DapLink(Link):
    ...
    def connect(self):
        ...
        self._dap_mode = Link.DP_MODE(self._protocol.connect())
        # set clock frequency
        self._protocol.setSWJClock(self._frequency)
        # configure transfer
        self._protocol.transferConfigure()
        if self._dap_mode == Link.DP_MODE.SWD:
            # configure swd protocol
            self._protocol.swdConfigure()
            # switch from jtag to swd
            self._jtag_to_swd()
            # read ID code
            logger.info('IDCODE: 0x%X', self.read_reg(DP_REG['IDCODE']))
            # clear errors
            self._protocol.writeAbort(0x1e)
        elif self._dap_mode == Link.DP_MODE.JTAG:
            # configure jtag protocol
            self._protocol.jtagConfigure(4)
            # Test logic reset, run test idle
            self._protocol.swjSequence([0x1F])
            # read ID code
            logger.info('IDCODE: 0x%X', self._protocol.jtagIDCode())
            # clear errors
            self.write_reg(DP_REG['CTRL_STAT'], CTRLSTAT_STICKYERR |
                           CTRLSTAT_STICKYCMP | CTRLSTAT_STICKYORUN)
        else:
            assert False
    ...
    def _clear_sticky_err(self):
        mode = self.get_swj_mode()
        if mode == Link.DP_MODE.SWD:
            self.write_reg(Link.REG.DP_0x0, (1 << 2))
        elif mode == Link.DP_MODE.JTAG:
            self.write_reg(DP_REG['CTRL_STAT'], CTRLSTAT_STICKYERR)
        else:
            assert False
