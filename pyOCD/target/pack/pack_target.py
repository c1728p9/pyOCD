...
def _validate_cache_targets(cache):
    """Check all targets in the cache and return True if usable"""
    bad_targets = []
    for name, dev in cache.index.items():
        if name == "version":
            continue
        try:
            memory_regions = list(dev["memory"].items())
            for region_name, info in memory_regions:
                int(info["start"], 0)
                int(info["size"], 0)
                assert (region_name.find("IROM") >= 0) or \
                       (region_name.find("IRAM") >= 0)
        except Exception:
