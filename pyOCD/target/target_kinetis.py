...
class Kinetis(CortexM):
    ...
    def init(self):
        ...
        # check MDM-AP ID
        val = self._temp_cmsis_dap.readAP(MDM_IDR)
        if val != self.mdm_idr:
        ...
        if isLocked:
            if self.do_auto_unlock:
                ...
                self._temp_cmsis_dap.writeAP(MDM_CTRL, MDM_CTRL_DEBUG_REQUEST)
                # Enable debug
                self.writeMemory(CortexM.DHCSR, CortexM.DBGKEY | CortexM.C_DEBUGEN)
                self.link.assert_reset(False)
                while self._temp_cmsis_dap.readAP(MDM_STATUS) & MDM_STATUS_CORE_HALTED != MDM_STATUS_CORE_HALTED:
                    logging.debug("Waiting for mdm halt (erase)")
                    sleep(0.01)

                # release MDM halt once it has taken effect in the DHCSR
                self._temp_cmsis_dap.writeAP(MDM_CTRL, 0)
        ...
        if self.halt_on_connect:
            ...
            self._temp_cmsis_dap.writeAP(MDM_CTRL, MDM_CTRL_DEBUG_REQUEST | MDM_CTRL_CORE_HOLD_RESET)
            while self._temp_cmsis_dap.readAP(MDM_CTRL) & (MDM_CTRL_DEBUG_REQUEST | MDM_CTRL_CORE_HOLD_RESET) != (MDM_CTRL_DEBUG_REQUEST | MDM_CTRL_CORE_HOLD_RESET):
                self._temp_cmsis_dap.writeAP(MDM_CTRL, MDM_CTRL_DEBUG_REQUEST | MDM_CTRL_CORE_HOLD_RESET)
            # Enable debug
            self.writeMemory(CortexM.DHCSR, CortexM.DBGKEY | CortexM.C_DEBUGEN)
    ...
    def isLocked(self):
        val = self._temp_cmsis_dap.readAP(MDM_STATUS)
    ...
    def massErase(self):
        ...
        while True:
            status = self._temp_cmsis_dap.readAP(MDM_STATUS)
        ...
        self._temp_cmsis_dap.writeAP(MDM_CTRL, MDM_CTRL_FLASH_MASS_ERASE_IN_PROGRESS)
        ...
        while True:
            val = self._temp_cmsis_dap.readAP(MDM_CTRL)
            if (val == 0):
                break
            sleep(0.01)
