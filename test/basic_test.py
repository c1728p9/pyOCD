...
def basic_test(board_id, file, log_func=default_log):
    ...
    with MbedBoard.chooseBoard(board_id=board_id) as board:
        ...
        log_func("binary file: %s" % binary_file)
        ...
        log_func("\r\n\r\n------ GET Unique ID ------")
        log_func("Unique ID: %s" % board.getUniqueID())

        log_func("\r\n\r\n------ TEST READ / WRITE CORE REGISTER ------")
        pc = target.readCoreRegister('pc')
        log_func("initial pc: 0x%X" % target.readCoreRegister('pc'))
        # write in pc dummy value
        target.writeCoreRegister('pc', 0x3D82)
        log_func("now pc: 0x%X" % target.readCoreRegister('pc'))
        # write initial pc value
        target.writeCoreRegister('pc', pc)
        log_func("initial pc value rewritten: 0x%X" % target.readCoreRegister('pc'))

        msp = target.readCoreRegister('msp')
        psp = target.readCoreRegister('psp')
        log_func("MSP = 0x%08x; PSP = 0x%08x" % (msp, psp))
        ...
        log_func("CONTROL = 0x%02x; FAULTMASK = 0x%02x; BASEPRI = 0x%02x; PRIMASK = 0x%02x" % (control, faultmask, basepri, primask))

        target.writeCoreRegister('primask', 1)
        newPrimask = target.readCoreRegister('primask')
        log_func("New PRIMASK = 0x%02x" % newPrimask)
        target.writeCoreRegister('primask', primask)
        newPrimask = target.readCoreRegister('primask')
        log_func("Restored PRIMASK = 0x%02x" % newPrimask)

        if target.has_fpu:
            s0 = target.readCoreRegister('s0')
            log_func("S0 = %g (0x%08x)" % (s0, float32beToU32be(s0)))
            target.writeCoreRegister('s0', math.pi)
            newS0 = target.readCoreRegister('s0')
            log_func("New S0 = %g (0x%08x)" % (newS0, float32beToU32be(newS0)))
            target.writeCoreRegister('s0', s0)
            newS0 = target.readCoreRegister('s0')
            log_func("Restored S0 = %g (0x%08x)" % (newS0, float32beToU32be(newS0)))
        ...
        log_func("halt")
        target.halt()
        log_func("HALT: pc: 0x%X" % target.readCoreRegister('pc'))
        sleep(0.2)


        log_func("\r\n\r\n------ TEST STEP ------")

        log_func("reset and halt")
        target.resetStopOnReset()
        currentPC = target.readCoreRegister('pc')
        log_func("HALT: pc: 0x%X" % currentPC)
        sleep(0.2)
        ...
        target.halt()
        log_func("READ32/WRITE32")
        val = randrange(0, 0xffffffff)
        log_func("write32 0x%X at 0x%X" % (val, addr))
        target.writeMemory(addr, val)
        res = target.readMemory(addr)
        log_func("read32 at 0x%X: 0x%X" % (addr, res))
        if res != val:
            log_func("ERROR in READ/WRITE 32")

        log_func("\r\nREAD16/WRITE16")
        val = randrange(0, 0xffff)
        log_func("write16 0x%X at 0x%X" % (val, addr + 2))
        target.writeMemory(addr + 2, val, 16)
        res = target.readMemory(addr + 2, 16)
        log_func("read16 at 0x%X: 0x%X" % (addr + 2, res))
        if res != val:
            log_func("ERROR in READ/WRITE 16")

        log_func("\r\nREAD8/WRITE8")
        val = randrange(0, 0xff)
        log_func("write8 0x%X at 0x%X" % (val, addr + 1))
        target.writeMemory(addr + 1, val, 8)
        res = target.readMemory(addr + 1, 8)
        log_func("read8 at 0x%X: 0x%X" % (addr + 1, res))
        if res != val:
            log_func("ERROR in READ/WRITE 8")
        ...
        for i in range(5):
            target.step()
            log_func("pc: 0x%X" % target.readCoreRegister('pc'))
